...
function evaluateDiplomacyProposal(room, aiId, proposal) {
  ...
  // TEAM MODE: permanent teams cannot form ANY pact (alliance OR non-aggression)
  // with outside factions, and teammates cannot pact each other. Reject outright.
  const gs = room.gameState;
  const proposerId = proposal.sender || proposal.proposerId;
  if (gs && proposerId && (
    GameEngine.isSameTeam(gs, aiId, proposerId) ||
    (GameEngine.isTeamMode(gs) && (GameEngine.isOnTeam(gs, aiId) || GameEngine.isOnTeam(gs, proposerId)))
  )) {
    return false;
  }
...
function evaluateDiplomaticProposalsToSend(room, aiId) {
  ...
  // TEAM MODE: a team member can never form any pact — skip diplomacy entirely
  if (GameEngine.isTeamMode(gameState) && GameEngine.isOnTeam(gameState, aiId)) {
    return null;
  }

  // Already has a pending proposal — do not spam. (The server also blocks
  // duplicates, but this prevents the AI from even generating redundant
  // proposal actions every runAITurn call, which was causing 60+ proposals
  // to the same player in a single turn.)
  const hasPendingProposal = gameState.diplomacyProposals?.some(p => p.proposerId === aiId);
  ...
  if (isDesperate && Math.random() < 0.50) {
    ...
    owned.forEach(tid => {
      GameEngine.getAdjacentTerritories(mapData.connections, tid).forEach(adjId => {
        const adjTerr = gameState.territories[adjId];
        if (adjTerr && adjTerr.ownerId !== null && adjTerr.ownerId !== aiId && !GameEngine.isSameTeam(gameState, aiId, adjTerr.ownerId)) {
  ...
  for (const nId of neighborPlayers) {
    ...
    // Skip teammates (never pact with your own team)
    if (GameEngine.isSameTeam(gameState, aiId, nId)) continue;
